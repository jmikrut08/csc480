    ...
    while s.size() > 0:
        ...
        if node.getState() not in visitedStates:
            ...
            for child in children:
                #print(child.getKey(), ":", child.getState(), ":", child.getDepth(), ":", child.getAction())
                if child.getState() != node.getState():
                    if child.getState() != node.getParentState():
                        ...
                        if key not in node.getChildKeys():
                            ...
                            print(child.getKey(), ":", child.getState(), ":", child.getDepth(), ":", child.getAction(), ", not included into the the stack not added into the stack")
